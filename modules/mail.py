...
def mail_BOT_request():
  ...
  return [counter, info_with_files]
...
def CheckMail():
  ...
  result, data = email.uid(
    'search', 'UNSEEN',
    'FROM "anketa@sut.ru"')  # Собираем UID писем НЕПРОЧИТАННЫХ от СПбГУТ
  mails = data[0].split()
  counter = 0
  full_info = {}
  for i in mails:
    ...
    if HasFile:
      text = text.replace(
        'Прикрепленные файлы: смотрите в личном кабинете СПбГУТ.', '')
